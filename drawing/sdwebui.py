...
class SDWebUI(DrawingAPI):
    ...
    async def img_to_img(self, init_images: List[Image], prompt=''):
        # You need to call the get_bytes method to get the base64 field content
        for x in init_images: await x.get_bytes()
        # The message chain displays the word "[Picture]" in the string and needs to be filtered.
        prompt = prompt.replace("[Picture]", "")
        payload = {
